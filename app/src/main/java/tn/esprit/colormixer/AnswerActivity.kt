...
class AnswerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // You must check if only one radio button is selected,
        // the navigate to the ResultActivity with the data name and RESULT (FAILED/SUCCESS)
        binding.btnSubmit.setOnClickListener(){
            if (!binding.rbGreen.isChecked && !binding.rbOrange.isChecked && !binding.rbPurple.isChecked){
                Snackbar.make(binding.contextView, "you must choose a color to answer", Snackbar.LENGTH_SHORT).show()
                //sortir de la gestion de clic, car il n'est pas nécessaire de poursuivre si l'utilisateur n'a pas fait de sélection
            ...
            // finish() : fermer l'activité actuelle après avoir démarré l'activité ResultActivity.
            // Cela peut être utile pour libérer des ressources et nettoyer l'activité actuelle.
            finish()
        }
